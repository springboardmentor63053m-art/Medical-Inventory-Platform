...
public class ReportController {
    ...
    @GetMapping
    public ResponseEntity<List<Report>> getAll() { return ResponseEntity.ok(service.findAll()); }

    @GetMapping("/{id}")
    public ResponseEntity<Report> getById(@PathVariable Long id) { return ResponseEntity.ok(service.findById(id)); }

    @PostMapping
    public ResponseEntity<Report> create(@RequestBody Report entity) { return ResponseEntity.ok(service.save(entity)); }
