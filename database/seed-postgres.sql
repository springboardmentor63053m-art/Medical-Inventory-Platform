-- ============================================================
-- MediStock Database Seed Data — PostgreSQL variant
-- (for Render's managed Postgres / any production Postgres database)
-- Data-identical to database/seed.sql (the MySQL version used for local
-- dev) — only the MySQL-specific control statements were adapted:
--  * no USE statement (Postgres connects to a database by connection
--    string, not by USE)
--  * no SET SQL_SAFE_UPDATES (MySQL-only safety toggle)
--  * ALTER TABLE ... AUTO_INCREMENT = 1  ->  ALTER TABLE ... ALTER COLUMN
--    id RESTART WITH 1 (assumes Hibernate created the tables with
--    GenerationType.IDENTITY, i.e. Postgres "GENERATED ... AS IDENTITY"
--    columns — true if you let Hibernate create the schema, see the
--    deployment guide)
--  * CURDATE()/DATE_ADD(...) -> CURRENT_DATE / INTERVAL arithmetic
--
...
-- script is about to replace. The database blocks the DELETE on medicines
-- with a foreign-key-violation error until those referencing rows are cleared first.
--
...
ALTER TABLE sale_items ALTER COLUMN id RESTART WITH 1;
ALTER TABLE sales ALTER COLUMN id RESTART WITH 1;
ALTER TABLE stock_movements ALTER COLUMN id RESTART WITH 1;
ALTER TABLE purchases ALTER COLUMN id RESTART WITH 1;
ALTER TABLE medicines ALTER COLUMN id RESTART WITH 1;
ALTER TABLE suppliers ALTER COLUMN id RESTART WITH 1;
