...
export default function UserDashboardPage() {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-900 pb-10">
      {/* Welcome Hero Banner */}
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-950 via-slate-900 to-indigo-950 p-8 rounded-3xl text-white shadow-xl border border-slate-800">
        ...
        <div className="relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-500/20 border border-blue-400/30 text-blue-300 font-bold text-xs rounded-full mb-3">
          ...
          <div className="mt-6 relative max-w-xl">
            <Search className="w-5 h-5 absolute left-4 top-1/2 -translate-y-12 text-slate-400" />
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search medicine by brand name, generic formulation, or code..."
              className="w-full pl-12 pr-4 py-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 shadow-inner"
            />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div
          ...
          className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs hover:shadow-md hover:border-blue-300 transition cursor-pointer flex items-center justify-between group"
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-900">Browse Medicines</h3>
              <p className="text-[11px] text-slate-500 mt-0.5">Explore full medicine catalog</p>
            </div>
          </div>
        ...
        <div
          ...
          className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs hover:shadow-md hover:border-purple-300 transition cursor-pointer flex items-center justify-between group"
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-0.5">Pharmaceutical classifications</p>
            </div>
          </div>
          <ArrowRight className="w-4 h-4 text-slate-400 group-hover:translate-x-1 transition" />
        </div>
        ...
        <div
          ...
          className="p-5 bg-white rounded-2xl border border-slate-200 shadow-xs hover:shadow-md hover:border-emerald-300 transition cursor-pointer flex items-center justify-between group"
        >
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold group-hover:scale-110 transition">
              <User className="w-6 h-6" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-900">My Profile</h3>
              <p className="text-[11px] text-slate-500 mt-0.5">Manage personal information</p>
            </div>
          </div>
          <ArrowRight className="w-4 h-4 text-slate-400 group-hover:translate-x-1 transition" />
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-100">
          <div>
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              <Pill className="w-5 h-5 text-blue-600" /> Medicine Formulations Catalog
            </h2>
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-slate-500 space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-700">No medicine formulations currently available in the catalog.</p>
            <button
              onClick={fetchUserData}
              className="mt-2 px-4 py-2 bg-blue-50 text-blue-600 text-xs font-bold rounded-xl hover:bg-blue-100 transition"
            >
              Refresh Catalog
            </button>
          </div>
          ...
          <div className="py-12 text-center text-slate-500 space-y-2">
            <p className="text-sm font-semibold text-slate-700">No medicines match your search criteria or category filter.</p>
            <button
              onClick={() => {
                setSearchTerm('');
                setSelectedCategory('');
                ...
                <div>
                  ...
                  <div className="mt-3 space-y-1 text-xs text-slate-600 border-t border-slate-200/60 pt-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-slate-700">{med.manufacturer || 'Standard'}</span>
                    </div>
                  </div>
                </div>
        ...
        {filteredMedicines.length > 9 && (
          <div className="text-center pt-2">
            <Link
              to="/medicines"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold rounded-xl transition"
            >
