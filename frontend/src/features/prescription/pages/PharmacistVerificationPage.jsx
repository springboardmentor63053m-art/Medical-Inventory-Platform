...
export default function PharmacistVerificationPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-indigo-50 via-blue-50 to-white dark:from-slate-900 dark:via-indigo-950 dark:to-slate-900 rounded-3xl p-6 lg:p-8 border border-indigo-100 dark:border-slate-800 shadow-xs dark:shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-600/10 rounded-full blur-3xl pointer-events-none" />
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-indigo-100 dark:bg-indigo-600/20 border border-indigo-200 dark:border-indigo-500/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shadow-sm">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <span className="px-2.5 py-0.5 rounded-full bg-indigo-100 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 text-[10px] font-bold text-indigo-700 dark:text-indigo-400 uppercase tracking-wider">
            ...
            {['PENDING_REVIEW', 'APPROVED', 'REJECTED', 'ALL'].map((st) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition ${
                  statusFilter === st ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                }`}
              >
                {st.replace('_', ' ')}
              </button>
      ...
      <div className="bg-white dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl p-6 border border-slate-200 dark:border-slate-800/80 shadow-xs dark:shadow-xl space-y-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
          <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
            <Clock className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Queue Orders ({filteredOrders.length})
          </h2>
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white"
            />
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-16 flex flex-col items-center justify-center text-slate-400 gap-2">
            ...
            <span className="text-xs">Fetching prescription queue from database...</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredOrders.map((ord) => (
              <div
                key={ord.id}
                className="p-5 rounded-2xl bg-slate-50/70 hover:bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 hover:border-indigo-400/50 transition flex flex-col justify-between gap-4 group shadow-2xs"
              >
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
                    <span className="text-xs text-slate-500 dark:text-slate-400">{ord.items?.length || 0} items</span>
                    <span className="text-sm font-black text-emerald-600 dark:text-emerald-400">₹{(ord.totalAmount || 0).toFixed(2)}</span>
                  </div>
                </div>

                <button
                  onClick={() => openReviewModal(ord)}
                  className="w-full py-2.5 rounded-xl bg-indigo-50 hover:bg-indigo-600 dark:bg-indigo-600/20 dark:hover:bg-indigo-600 border border-indigo-200 dark:border-indigo-500/30 text-indigo-700 hover:text-white dark:text-indigo-300 dark:hover:text-white text-xs font-bold transition flex items-center justify-center gap-2 shadow-2xs"
                >
        ...
        >
          <div className="space-y-5 py-2 text-xs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Order Number</span>
                  ...
                  <div>
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 inline-block">
                      {selectedOrder.status}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Created Date & Time</span>
                  ...
                  <div className="col-span-2 pt-1 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    ...
                    <span className="text-base font-black text-emerald-600 dark:text-emerald-400">₹{(selectedOrder.totalAmount || 0).toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Customer Name</span>
                    <strong className="text-slate-900 dark:text-slate-200">{selectedOrder.userFullName || 'N/A'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">User ID / Email</span>
                    <span className="text-slate-700 dark:text-slate-300 truncate block">#{selectedOrder.userId} • {selectedOrder.userEmail}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Prescribing Doctor</span>
                    <strong className="text-slate-900 dark:text-white">{selectedOrder.doctorName || 'Not specified'}</strong>
                  </div>
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Delivery Address</span>
                    <span className="text-slate-700 dark:text-slate-300 block truncate">{selectedOrder.deliveryAddress}</span>
                  </div>
                  <div>
                    <span className="text-slate-500 dark:text-slate-400 block">Contact Phone</span>
                    <span className="text-slate-700 dark:text-slate-300 font-mono">{selectedOrder.contactPhone}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
                <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                  <FileText className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> Prescribed Document (PostgreSQL BYTEA Storage)
                </h4>
                {docBlobUrl && (
                  <div className="flex items-center gap-2">
                    <a
                      ...
                      className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 flex items-center gap-1 text-xs font-bold bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-200 dark:border-emerald-500/20"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-indigo-700 dark:text-indigo-400 hover:text-indigo-800 flex items-center gap-1 text-xs font-bold bg-indigo-50 dark:bg-indigo-500/10 px-2.5 py-1 rounded-lg border border-indigo-200 dark:border-indigo-500/20"
                    >
              ...
              {selectedOrder.prescriptionFileUrl ? (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Filename</span>
                      <strong className="text-slate-800 dark:text-slate-200 truncate block">{selectedOrder.prescriptionFileName || 'prescription-doc'}</strong>
                    </div>
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Content Type</span>
                      <strong className="text-indigo-600 dark:text-indigo-400 block">{selectedOrder.prescriptionContentType || 'Document'}</strong>
                    </div>
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">File Size</span>
                    ...
                    <div>
                      <span className="text-slate-500 dark:text-slate-400 block">Upload Date</span>
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-2 min-h-48 flex items-center justify-center">
                    {loadingDoc ? (
                      ...
                      (selectedOrder.prescriptionContentType?.toLowerCase().includes('pdf') || selectedOrder.prescriptionFileName?.toLowerCase().endsWith('.pdf')) ? (
                        <object data={docBlobUrl} type="application/pdf" className="w-full h-96 rounded-lg border border-slate-200 dark:border-slate-800">
                          <iframe src={docBlobUrl} title="Prescription PDF" className="w-full h-96 rounded-lg border border-slate-200 dark:border-slate-800">
                            <div className="p-4 text-center text-xs text-slate-500">
                              PDF inline display unsupported by browser.{' '}
                              <a href={docBlobUrl} target="_blank" rel="noreferrer" className="text-indigo-600 underline font-bold">
                        ...
                        <img
                          ...
                          className="max-h-96 max-w-full rounded-lg object-contain border border-slate-200 dark:border-slate-800 shadow-md"
                        />
                      )
                    ) : (
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 italic bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-2">
              <h4 className="text-xs font-bold text-slate-800 dark:text-slate-300 flex items-center justify-between">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {item.prescriptionRequired ? (
                          <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30">Rx</span>
                        ) : (
                          <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30">OTC</span>
                        )}
                      </div>
                      ...
                      <p className="text-[10px] text-slate-500 mt-0.5">
                        Current DB Stock: <strong className={item.currentStock >= item.quantity ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}>{item.currentStock ?? 'In Stock'} units</strong>
                      </p>
                    </div>

                    <div className="text-right flex-shrink-0">
                      <span className="font-bold text-slate-700 dark:text-slate-200">Qty: {item.quantity} × ₹{(item.unitPrice || 0).toFixed(2)}</span>
                      <p className="text-sm font-black text-emerald-600 dark:text-emerald-400">₹{(item.subtotal || 0).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              />
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                disabled={actionLoading}
                className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 shadow-md shadow-emerald-600/20 transition disabled:opacity-50"
              >
