...
export default function PrescriptionOrderPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-blue-50 via-white to-indigo-50 dark:from-slate-900 dark:via-indigo-950 dark:to-slate-900 rounded-3xl p-6 lg:p-8 border border-blue-200 dark:border-slate-800 shadow-xl dark:shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-600/10 rounded-full blur-3xl pointer-events-none" />

        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-blue-100 dark:bg-blue-600/20 border border-blue-200 dark:border-blue-500/30 flex items-center justify-center text-blue-600 dark:text-blue-400 shadow-xl">
          ...
          <div className="flex items-center gap-2 bg-white/80 dark:bg-slate-900/80 p-1.5 rounded-2xl border border-blue-200 dark:border-slate-800">
            <button
              onClick={() => setActiveTab('new')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 ${
                activeTab === 'new'
                  ? 'bg-blue-600 text-white shadow-lg'
                  : 'text-slate-600 hover:text-blue-700 dark:text-slate-400 dark:hover:text-white'
              }`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              {loadingMeds ? (
                ...
                <div className="space-y-2.5 max-h-[500px] overflow-y-auto pr-1">
                  ...
                  {filteredMedicines.map((med) => (
                    <div
                      key={med.id}
                      className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/80 hover:border-blue-400 dark:hover:border-blue-500/30 transition flex items-center justify-between gap-3 group"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {med.prescriptionRequired ? (
                            ...
                            <span className="p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/30 text-xs text-amber-800 dark:text-amber-200">
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="bg-white dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl p-6 border border-slate-200 dark:border-slate-800/80 shadow-xl space-y-5">
              ...
              {cart.length > 0 && (
                hasRxItemInCart ? (
                  <div className="p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-300 dark:border-amber-500/30 text-amber-800 dark:text-amber-200 text-xs space-y-1">
                  ...
                  <div className="p-3.5 rounded-2xl bg-blue-50 dark:bg-blue-500/10 border border-blue-300 dark:border-blue-500/30 text-blue-800 dark:text-blue-200 text-xs space-y-1">
                    <div className="flex items-center gap-2 font-bold text-blue-800 dark:text-blue-300">
                      <CheckCircle2 className="w-4 h-4 text-blue-400" /> No Prescription Required
                    </div>
              ...
              {cart.length === 0 ? (
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-slate-950/60 rounded-xl border border-slate-200 dark:border-slate-800 text-xs">
                      <div className="flex-1 min-w-0 pr-2">
                        <p className="font-semibold text-slate-900 dark:text-slate-200 truncate flex items-center gap-1.5">
                          {item.name}
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex items-center bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded-lg">
                          <button onClick={() => updateQuantity(item.id, -1)} className="px-2 py-0.5 text-slate-600 hover:text-blue-700 dark:text-slate-400 dark:hover:text-white">-</button>
                          <span className="px-2 text-slate-900 dark:text-slate-200 font-bold">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, 1)} className="px-2 py-0.5 text-slate-600 hover:text-blue-700 dark:text-slate-400 dark:hover:text-white">+</button>
                        </div>
                  ...
                  <div className="pt-2 flex justify-between text-sm font-bold text-slate-900 dark:text-white border-t border-slate-200 dark:border-slate-800">
                    ...
                    <span className="text-blue-700 dark:text-blue-300">₹{calculateTotal()}</span>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  {prescriptionFile && (
                    <div className="flex items-center justify-between text-[11px] text-slate-700 dark:text-slate-300 bg-blue-50 dark:bg-slate-950/80 p-2 rounded-xl border border-blue-200 dark:border-slate-800">
                      <span className="truncate flex items-center gap-1.5 text-blue-700 dark:text-blue-300">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-4">
              {myOrders.map((ord) => (
                <div key={ord.id} className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 space-y-3">
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 dark:border-slate-800/80 pb-3">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-1">
                        Patient: <strong className="text-slate-800 dark:text-slate-200">{ord.patientName || ord.userFullName || 'Patient information unavailable'}</strong> • Placed on {new Date(ord.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {ord.items?.map((item) => (
                      <div key={item.id} className="p-2.5 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/60 flex items-center justify-between text-xs">
                        <div>
                          <p className="font-bold text-slate-900 dark:text-slate-200">{item.medicineName}</p>
                  ...
                  {ord.pharmacistNotes && (
                    <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 text-xs text-blue-800 dark:text-blue-300">
