...
export default function PurchaseOrderPage() {
  ...
  const handleLineItemChange = (index, field, value) => {
    ...
    if (field === 'medicineId') {
      const medicine = supplierMedicines.find(item => String(item.id) === String(value)) || medicines.find(item => String(item.id) === String(value));
      updated[index].unitPrice = Number(medicine?.costPrice ?? medicine?.unitPrice ?? 0);
    }
  ...
  const openReceiveModal = (order) => {
    const preparedItems = (order.items || []).map((item) => {
      const inventoryInfo =
        inventoryByMedicine[String(item.medicineId)] || {};
      ...
      return {
        ...
        expiryDate: '',
        storageLocation: '',
        minimumStock: Number(inventoryInfo.minimumStock || 10),
      };
    });

    setReceivingOrder(order);
    setReceiptItems(preparedItems);
  ...
  const handleReceiveOrder = async (event) => {
    ...
    const hasMissingDetails = receiptItems.some(
      (item) => !item.batchNumber.trim() || !item.expiryDate
    );

    if (hasMissingDetails) {
      toast.error(
        'Enter a batch number and expiry date for every item.'
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        items: receiptItems.map((item) => ({
          ...
          storageLocation:
            item.storageLocation.trim() || 'Main Warehouse',
  ...
  const renderSortIcon = (field) => {
    ...
    switch (status) {
      ...
      case 'PROCESSING':
        return (
          <span className="px-2.5 py-1 bg-purple-100 text-purple-800 font-bold text-[10px] rounded-full border border-purple-200 inline-flex items-center gap-1">
      ...
      case 'SHIPPED':
        return (
          <span className="px-2.5 py-1 bg-cyan-100 text-cyan-800 font-bold text-[10px] rounded-full border border-cyan-200 inline-flex items-center gap-1">
          ...
          <div className="bg-white w-full max-w-xl rounded-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[calc(100vh-2rem)] overflow-hidden">
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50/80 shrink-0 flex flex-wrap items-center justify-between gap-2">
              ...
              {viewOrder.status === 'PENDING' &&
                ...
                (
                  !viewOrder.createdBy ||
                  viewOrder.createdBy.toLowerCase() !==
                    user?.email?.toLowerCase()
                ) && (
                  <button
                    onClick={() =>
                      handleUpdateStatus(viewOrder.id, 'APPROVED')
                    }
              ...
              {viewOrder.status === 'APPROVED' && isSupplier && (
                <button
                  onClick={() =>
                    handleUpdateStatus(viewOrder.id, 'PROCESSING')
                  }
                  className="px-3.5 py-2 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl text-xs shadow-xs transition"
                >
                  Start Processing
                </button>
              )}

              {viewOrder.status === 'PROCESSING' && isSupplier && (
                <button
                  onClick={() =>
                    handleUpdateStatus(viewOrder.id, 'SHIPPED')
                  }
                  className="px-3.5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white font-bold rounded-xl text-xs shadow-xs transition"
                >
                  Mark as Shipped
                </button>
              )}
              ...
              {viewOrder.status === 'SHIPPED' &&
                (isAdmin || isPharmacist) && (
                  <button
                    onClick={() => openReceiveModal(viewOrder)}
            ...
            {receiveModalOpen && receivingOrder && (
        <div className="fixed inset-0 z-[70] bg-slate-950/50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-3xl max-h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col">
            <div className="p-5 border-b border-slate-200 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Enter the batch and expiry information before adding stock.
                </p>
              </div>
            ...
            >
              <div className="p-5 space-y-4 overflow-y-auto">
                {receiptItems.map((item, index) => (
                  <div
                    ...
                    className="p-4 border border-slate-200 rounded-2xl bg-slate-50/60"
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <label className="text-xs font-semibold text-slate-600">
                        ...
                        <input
                          ...
                          placeholder="Example: BAT-2026-501"
                          className="mt-1.5 w-full px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                      </label>
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-end gap-3">
                <button
                  ...
                  className="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-bold hover:bg-slate-100 disabled:opacity-50"
                >
                  Cancel
                </button>
                ...
                <button
                  type="submit"
                  disabled={receiving || receiptItems.length === 0}
                  className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-2 disabled:opacity-50"
                >
                  {receiving ? (
                    <>
