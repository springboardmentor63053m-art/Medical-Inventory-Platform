    ...
    <>
      ...
      >
        ...
        <div className="relative flex items-center justify-between px-5 py-5 border-b border-white/[0.08]">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-10 h-10 bg-[#c2674a] rounded-[4px]
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="px-1.5 py-0.5 rounded-[2px] bg-[#c2674a] text-white font-black text-[9px] tracking-wider uppercase">
                  AI
                </span>
              </div>
              <p className="text-white/60 text-[10px] font-semibold tracking-wider uppercase">Pharmacy Platform</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="lg:hidden text-white/50 hover:text-white p-1.5 rounded-[4px] hover:bg-white/10
                       transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="px-4 py-3.5 border-b border-white/[0.08]">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-[4px]
                          bg-white/[0.05] border border-white/[0.08]">
            <div className="relative flex-shrink-0">
              <div className="w-8 h-8 rounded-[4px] bg-[#c2674a]
                              flex items-center justify-center text-white font-bold text-xs">
                {user?.username?.[0]?.toUpperCase()}
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-[#4a7c59]
                               rounded-full border-2 border-[#1c3829]" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-white text-xs font-semibold truncate">{user?.username}</p>
              <p className="text-white/60 text-[10px] truncate capitalize font-medium">{roleLabel}</p>
            </div>
            <Zap className="w-3.5 h-3.5 text-[#c2674a] flex-shrink-0" />
          </div>
        </div>
          ...
          {navSections.map((section) => {
            ...
            return (
              <div key={section.title} className="space-y-1">
                <p className="text-white/40 text-[10px] font-bold uppercase tracking-[0.14em] px-3 mb-1.5">
                  {section.title}
                </p>
                ...
                {visibleItems.map(({ to, label, icon: Icon }) => (
                  ...
                  >
                    {({ isActive: navActive }) => (
                      <span className={`
                        flex items-center gap-2.5 px-3 py-2 rounded-full text-xs font-semibold
        ...
        <div className="px-4 py-3.5 border-t border-white/[0.08]">
          <div className="flex items-center justify-between px-2">
            <div className="flex items-center gap-2">
              ...
              <p className="text-white/60 text-[11px] font-semibold">MediStock AI v3.0</p>
            </div>
            <span className="text-[10px] text-[#c2674a] font-mono font-bold">AI Active</span>
          </div>
        </div>
      </aside>
    </>
