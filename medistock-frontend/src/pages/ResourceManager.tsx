...
export type ResourceConfig = { title: string; singular: string; endpoint: string; fields: Field[]; searchEndpoint?: string; special?: "inventory" | "notifications" | "expiries" };
...
export function ResourceManager({ config }: { config: ResourceConfig }) {
  ...
  const load = useCallback(async () => {
    try { setLoading(true); setRows(asRows(await service.getAll())); }
    catch (err) { toast.error(extractError(err, `Unable to load ${config.title.toLowerCase()}`)); }
    finally { setLoading(false); }
  }, [config.title, service]);

  useEffect(() => { load(); }, [load]);
  useEffect(() => {
    const refs = [...new Map(config.fields.filter(f => f.reference).map(f => [f.reference!.endpoint, f.reference!])).entries()];
  ...
  const performSearch = async () => {
    ...
    try { setLoading(true); const { data } = await axiosInstance.get(config.searchEndpoint, { params: { name: query } }); setRows(asRows(data)); }
    catch (err) { toast.error(extractError(err, "Search failed")); } finally { setLoading(false); }
  };
  ...
  const filtered = config.searchEndpoint ? safeRows : safeRows.filter(row => Object.values(row).some(v => String(typeof v === "object" ? displayValue(v, { key: "", label: "" }) : v).toLowerCase().includes(query.toLowerCase())));
