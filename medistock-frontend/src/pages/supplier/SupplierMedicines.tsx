...
const demoMedicines: Medicine[] = [
  { id: 1, name: "Metformin 500mg", category: "Diabetes", batchNumber: "MED-MET-001", quantity: 120, status: "IN STOCK" },
  { id: 2, name: "Amlodipine 5mg", category: "Blood Pressure", batchNumber: "MED-AML-001", quantity: 85, status: "IN STOCK" },
  { id: 3, name: "Vitamin C 500mg", category: "Vitamins", batchNumber: "MED-VIT-001", quantity: 35, status: "LOW STOCK" },
  { id: 4, name: "Paracetamol 500mg", category: "Pain Relief", batchNumber: "MED-PAR-002", quantity: 0, status: "OUT OF STOCK" },
];
...
export default function SupplierMedicines() {
  ...
  const user = getUser();
  const supplierName = String(user?.username || user?.name || user?.fullName || "Rahul")
    .replace(/_/g, " ")
  ...
  const loadMedicines = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(list) && list.length > 0) setMedicines(list);
      }
    } catch {
    ...
    <div className="supplier-app">
      <aside className="supplier-sidebar">
        ...
        <nav className="supplier-navigation">
          ...
          <button type="button" className="supplier-nav-item active" onClick={() => navigate("/supplier/medicines")}>
            <span>💊</span><strong>My Medicines</strong>
          </button>
          <button type="button" className="supplier-nav-item" onClick={() => navigate("/supplier/purchases")}>
      ...
      <main className="supplier-main">
        <header className="supplier-topbar">
          ...
          <div className="supplier-topbar-right">
            <button type="button" className="supplier-notification-button" onClick={() => navigate("/supplier/notifications")}>♧<span /></button>
        ...
        <div className="supplier-content">
          <section className="supplier-page-heading">
            <div>
              ...
              <h1>My Medicines</h1>
              <p>View medicines supplied by your supplier account and their current stock status.</p>
            </div>
            <button type="button" className="supplier-refresh" onClick={loadMedicines} disabled={loading}>
              ↻ {loading ? "Refreshing..." : "Refresh"}
            </button>
          </section>

          <section className="supplier-stat-grid">
            <div className="supplier-stat-card"><div className="supplier-stat-icon blue">💊</div><div><span>SUPPLIED MEDICINES</span><strong>{medicines.length}</strong><small>Medicines supplied</small></div></div>
            <div className="supplier-stat-card"><div className="supplier-stat-icon green">✓</div><div><span>IN STOCK</span><strong>{medicines.filter((m) => m.status === "IN STOCK").length}</strong><small>Available medicines</small></div></div>
            <div className="supplier-stat-card"><div className="supplier-stat-icon orange">!</div><div><span>LOW STOCK</span><strong>{medicines.filter((m) => m.status === "LOW STOCK").length}</strong><small>Needs attention</small></div></div>
            <div className="supplier-stat-card"><div className="supplier-stat-icon purple">0</div><div><span>OUT OF STOCK</span><strong>{medicines.filter((m) => m.status === "OUT OF STOCK").length}</strong><small>Currently unavailable</small></div></div>
          </section>

          <section className="supplier-card supplier-table-card">
            <div className="supplier-card-heading large">
              <div><h2>Supplied Medicines</h2><p>Medicines provided by your supplier account</p></div>
            ...
            <div className="supplier-table-wrapper">
              <table>
                <thead><tr><th>MEDICINE</th><th>CATEGORY</th><th>BATCH NUMBER</th><th>QUANTITY</th><th>STATUS</th></tr></thead>
                <tbody>
                  {filteredMedicines.map((medicine) => (
                    <tr key={medicine.id}>
                      <td><strong>{medicine.name}</strong></td>
                      <td>{medicine.category}</td>
                      <td><span className="supplier-batch">{medicine.batchNumber}</span></td>
                      <td><strong>{medicine.quantity}</strong></td>
                      <td><span className={`supplier-status ${medicine.status === "IN STOCK" ? "stock" : medicine.status === "LOW STOCK" ? "low" : "out"}`}><i />{medicine.status}</span></td>
                    </tr>
                  ))}
                  {filteredMedicines.length === 0 && (
                    <tr><td colSpan={5} className="supplier-empty-state">No medicines found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
